...
def double_eights(n: int) -> bool:
    ...
    if(n < 100):
        return last == 8 and second_last ==8
...
def help(n,x,cnt):
    if(x == 5):
        return int(cnt+count_digit(n,x)*(count_digit(n,x)-1)/2)
    return help(n,x+1,cnt+count_digit(n,x)*count_digit(n,10-x))
def ten_pairs(n):
...
def count_digit(n, digit):
    ...
    if(last == digit):
        tmp = 1
    return tmp+count_digit(n//10,digit)
