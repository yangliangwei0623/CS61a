...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    l = [s for s in paragraphs if(select(s))]
    if(k < len(l)):
        return l[k]
    else:
        return ''
...
def about(keywords: list[str]):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def check(paragraphy):
        paragraphy_tmp = remove_punctuation(paragraphy)
        words = split(paragraphy_tmp)
        words = [lower(s) for s in words]
        for s in words:
            if(s in keywords):
                return True
        return False
...
def accuracy(entered: str, source: str) -> float:
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 3
    if(len(source) == 0 and len(entered) == 0):
        return float(100)
    elif(len(entered) == 0):
        return float(0)
    elif(len(source) == 0):
        return float(0)
    cnt = 0
    source_lst = split(source)
    entered_lst = split(entered)
    for i in range(len(entered_lst)):
        if(i >= len(source_lst)):
            break
        if(entered_lst[i] == source_lst[i]):
            cnt += 1
    return cnt/len(entered_lst)*100.0
...
def autocorrect(entered_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    ans = entered_word
    for s in word_list:
        if(diff_function(entered_word,s,limit) < minimum):
            minimum = diff_function(entered_word,s,limit)
            ans = s
    return ans
...
def furry_fixes(entered: str, source: str, limit: int) -> int:
    ...
    def help(en,so,cnt,limit):
        ...
        if(so == ''):
            return len(en)+cnt
        if(en[0] == so[0]):
            return help(en[1:],so[1:],cnt,limit)
        else:
            return help(en[1:],so[1:],cnt+1,limit)
    return help(entered,source,0,limit)
...
def minimum_mewtations(entered: str, source: str, limit: int) -> int:
    ...
    def help(en,so,cnt,limit):
        ...
        if(en[0] == so[0]):
            ...
            return min(help(en[1:],so[1:],cnt+1,limit),help(en,so[1:],cnt+1,limit),
                       help(en[1:],so,cnt+1,limit))
    return help(entered,source,0,limit)
